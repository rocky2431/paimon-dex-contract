...
import {IPaimonV3SwapCallback} from "../v3-interfaces/callback/IPaimonV3SwapCallback.sol";
import {IPaimonV3FlashCallback} from "../v3-interfaces/callback/IPaimonV3FlashCallback.sol";
...
/// @title PaimonV3Pool
/// @notice A Paimon V3 pool implementing concentrated liquidity AMM
contract PaimonV3Pool is IPaimonV3Pool, NoDelegateCall {
    ...
    /// @dev Prevents calling a function from anyone except the address returned by IPaimonV3Factory#owner()
    modifier onlyFactoryOwner() {
        (bool success, bytes memory data) = factory.staticcall(abi.encodeWithSignature("owner()"));
        if (!success || msg.sender != abi.decode(data, (address))) revert NotOwner();
        _;
    }

    constructor() {
        int24 _tickSpacing;
        (factory, token0, token1, fee, _tickSpacing) = PaimonV3PoolDeployer(msg.sender).parameters();
        ...
        )
    {
...

        {
            ...
            if (!initializedLower) revert InvalidTick();
            ...
            if (!initializedUpper) revert InvalidTick();
        }
    ...
    function mint(
        ...
        IPaimonV3MintCallback(msg.sender).paimonV3MintCallback(amount0, amount1, data);
    ...
    function swap(
        ...
        if (zeroForOne) {
            ...
            IPaimonV3SwapCallback(msg.sender).paimonV3SwapCallback(amount0, amount1, data);
    ...
    function flash(
        ...
        IPaimonV3FlashCallback(msg.sender).paimonV3FlashCallback(fee0, fee1, data);
