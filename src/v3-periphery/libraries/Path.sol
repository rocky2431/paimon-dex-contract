...
library Path {
    ...
    function numPools(bytes memory path) internal pure returns (uint256) {
        // Ignore the first token address. From then on every fee and token offset
        // indicates a pool.
    ...
    /// @dev Read an address from a bytes array
    ...
    /// @dev Read a uint24 from a bytes array
    function toUint24(bytes memory _bytes, uint256 _start) internal pure returns (uint24 value) {
        require(_start + 3 >= _start, "toUint24_overflow");
        require(_bytes.length >= _start + 3, "toUint24_outOfBounds");
        assembly {
            value := mload(add(add(_bytes, 0x3), _start))
        }
    }
